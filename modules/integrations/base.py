...
``MockSystemClient`` loads a workbook's JSON snapshot (generated by
``snapshot_tool.py``) and returns the requested sheets in exactly the same shape
as the old ``xlsx_util.load_sheets`` — a ``{sheet_name: [row_dict, ...]}`` dict —
so validator logic is unchanged. Every fetch is recorded in ``CALL_LOG`` so the
demo can show which systems each agent talked to.
"""
import functools
import json
import os
...
def _load_snapshot(workbook: str) -> Dict[str, List[dict]]:
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Mock data snapshot '{os.path.basename(path)}' is missing. "
            f"Run: python -m modules.integrations.snapshot_tool")
